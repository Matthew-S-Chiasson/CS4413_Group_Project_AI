...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # Get the updated model parameters (weights) after training
        updated_parameters = self.get_parameters(config)

        # Simulate encryption of model parameters (weights)
        encrypted_parameters = []
        for param in updated_parameters:
            # Convert NumPy array to tensor (before encrypting)
            param_tensor = torch.tensor(param)
            
            # Flatten tensor to 1D to simulate encryption (as TenSEAL works with vectors)
            param_flattened = param_tensor.flatten().cpu().numpy()

            # Encrypt the flattened parameters using TenSEAL's context
            encrypted_param = ts.ckks_vector(self.context,param_flattened.tolist())  # Listify NumPy array
            encrypted_parameters.append(encrypted_param)

        original_shapes = [param.shape for param in updated_parameters]
        
        # Simulate decryption (optional, just for demonstration)
        decrypted_parameters = []
        for i, encrypted_param in enumerate(encrypted_parameters):
            decrypted_param = encrypted_param.decrypt()  # Decrypting the ciphertext
            # Reshape using the stored original shape
            decrypted_param_array = np.array(decrypted_param).reshape(original_shapes[i])
            decrypted_parameters.append(decrypted_param_array)
        
        # Return the "encrypted" parameters to simulate sending encrypted data
        return decrypted_parameters, len(self.train_loader.dataset), {}
